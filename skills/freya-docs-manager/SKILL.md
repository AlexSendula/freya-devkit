...
This skill creates and maintains comprehensive, standardized project documentation using a **coordinator + workers** architecture: one coordinator plans, then one
...
When you run `freya-docs-manager resolve`, the skill:
...
When you run `freya-docs-manager upgrade-diagrams`, the skill:
...
```
freya-docs-manager upgrade-diagrams
```
...
When the dependency-graph skill (`freya-code-graph` — registered as `freya-devkit:freya-code-graph` under a Claude plugin install) is available, docs-manager uses it for impact-aware updates:
...
Creates and maintains comprehensive, standardized project documentation using a coordinator + workers architecture.
...
freya-docs-manager init              # Initialize docs for a new project (includes review)
freya-docs-manager update            # Update all docs after changes (includes review)
freya-docs-manager update api        # Update just API.md (includes review)
freya-docs-manager review            # Review docs for consistency (standalone)
freya-docs-manager sync              # Full sync with current codebase (includes review)
freya-docs-manager resolve           # Resolve placeholders in existing docs
freya-docs-manager upgrade-diagrams  # Convert ASCII diagrams to mermaid
```

## Keeping Docs Updated

This skill uses **manual triggering** - run `freya-docs-manager update` after completing features or making significant changes. This gives you control over when documentation updates happen.

Recommended workflow:
1. Implement feature
2. Test and verify
3. Run `freya-docs-manager update` before committing
...
2. **Update with features** - Run `freya-docs-manager update` after completing work
