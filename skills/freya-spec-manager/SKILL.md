...
description: |
  ...
  INTEGRATION: Uses freya-code-graph skill (when available) for impact-aware spec updates.
...
This enables git-aware incremental updates via `freya-spec-manager update` (without arguments).
...
2. **Build the code graph.** Run `freya-code-graph build` — the shape detector needs it, and it is cheap and useful regardless of shape.
3. **Detect shape and recommend.** Run:
   ```bash
   freya project-shape --project . --format text
   ```
...
### `freya-spec-manager create <name>`
...
1. Scans codebase structure (file-pattern search for key paths)
...
returns its candidate specs for classification rather than writing files.*

Run them **in parallel — the default** — if your agent supports subagents (Claude
Code, Cursor, Codex and OpenCode all do, and so does Copilot — but Copilot's own
instructions tell it *not* to split a review by labeled area when the scope is
small enough to read directly, so by default it runs these itself, one at a time,
and may still describe them as parallel. Its `/fleet` mode enables parallel
subagent execution; this suite has not tested it). If it cannot, run them
one at a time, all of them, in order. On a small project the result is
the same, only slower; on a large one a sequential run accumulates every task's
reading context in a single window and may not fit, so narrow the scope — fewer
files, or fewer tasks per run — rather than letting it truncate.
...
### `freya-spec-manager update` (no arguments)
...
2. **When the dependency-graph skill (`freya-code-graph` — registered as `freya-devkit:freya-code-graph` under a Claude plugin install) is available:**
...
6. Group affected areas by category, one discovery task per group
...
### `freya-spec-manager update <spec>`
...
1. Run the deterministic link checks first — these are cheap and certain:
   ```bash
   freya verify-links --format json
   ```
...
### `freya-spec-manager adr create <name>`
...
### `freya-spec-manager search <query>`
...
### `freya-spec-manager by-tag <tag>`

Filter specs by tag:

1. Run `freya spec --tag <tag>`
2. Present results in table format
3. Offer to load full specs

### `freya-spec-manager get <id...>`
...
Examples:
- `freya-spec-manager get SPEC-001` - Get single spec
- `freya-spec-manager get SPEC-001 SPEC-003 SPEC-007` - Get multiple specs

### `freya-spec-manager review`

Interactive review of low-certainty specs with intelligent uncertainty handling.

**Phase 1: Discovery**
1. Run `freya spec --sort-certainty --below 100`
...
freya-spec-manager init                    # Initialize specs directory
freya-spec-manager create user-auth        # Create a new spec interactively
freya-spec-manager scan                    # Full codebase scan (first time or complete refresh)
freya-spec-manager update                  # Smart sync after code changes (recommended)
freya-spec-manager update SPEC-001         # Update a specific spec
freya-spec-manager search authentication   # Search specs for "authentication"
freya-spec-manager get SPEC-001            # Read a specific spec
freya-spec-manager verify                  # Check all specs are still accurate
```
...
- **Soft injection:** `freya principles list` surfaces it at design time (`create`, `scan`)
...
The `freya principles resolve` / `prior` subcommands back the checkpoint; `list` backs
soft injection.
...
**Usage:**
```bash
freya spec --query "authentication"
freya spec --tag security --min-certainty 70
freya spec --category auth
freya spec --id SPEC-001
freya spec --sort-certainty    # Lowest first
freya spec --below 100         # All below 100% certainty
```
...
```bash
freya adapters gherkin-scaffold \
...
When the dependency-graph skill (`freya-code-graph`) is available, spec-manager uses it for enhanced analysis:
...
**Enhanced scan mode:**
- Use `freya-code-graph dependents <file>` to suggest `related_code` entries for new specs
- Analyze dependency relationships to identify feature boundaries

**Fallback behavior:**
If the dependency-graph skill (`freya-code-graph`) is not available, use simple git diff analysis (current behavior).
