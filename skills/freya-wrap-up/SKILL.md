...
1. **Update dependency graph** - `freya-code-graph update` for impact-aware updates
2. **Update documentation** - `freya-docs-manager update` for project docs
3. **Update specifications** - `freya-spec-manager update` for feature specs
3.5. **Behavior integrity & run** - deterministic link checks + run *accepted* behaviors; **only deterministic failures block**
4. **Run security scan** - `freya-codebase-security-scan update` for incremental scan
...
```
freya-wrap-up
freya-wrap-up "feat: add user authentication"
```
...
Run `freya-code-graph update` to refresh the dependency graph with any code changes.
...
Run `freya-spec-manager update` to sync feature specifications.
...
7. **Declarative-drift check (governance P4b — resolve-to-proceed).** After the
   ...
   *declared* intent (spec `intentional_decisions` / spec prose, or an accepted ADR's
   ...
   targets / any tooling error — note it and continue. `freya drift gaps` is on-demand
...
Run `freya-codebase-security-scan update` for incremental security analysis.

This creates/updates the security report in `/knowledge-base/security/codebase-security/`.

> Note: `update` now includes lightweight in-loop adversarial verification of each finding (synchronous, free, in the main loop), so it composes cleanly inside this linear pipeline. Do **not** substitute `freya-codebase-security-scan audit` or `freya-codebase-security-scan scan` here — both drive paid agent CLIs (`freya security audit` / `freya security scan`) and must not run inside wrap-up. This includes the first wrap-up on a project, where there is no `.security-last-scan` yet: `update` covers that case with a full pass in the main loop, on purpose. Wrap-up spends nothing, and that has to stay true on run one as well as run fifty.
...
0. Advance the declared-intent baseline to the commit being wrapped (only reached
   ...
   ```bash
   freya verify-intent --project . --advance
   ```
...
This skill requires the following skills to be available (identified here by
capability; under a Claude plugin install each is registered as
`freya-devkit:<skill>` with the directory name unchanged — so `freya-code-graph`
is `freya-devkit:freya-code-graph`. The `freya-` prefix is **not** stripped at
install time, and looking for the unprefixed name finds nothing, which this skill
reports as a missing skill and silently skips):
- `freya-code-graph` - Dependency graph
- `freya-docs-manager` - Documentation management
- `freya-spec-manager` - Specification management
- `freya-behavior-graph` - Behavior graph (Phase 3.5 build/check/surface)
- `freya-behavior-runner` - Runs accepted behaviors, captures coverage
- `freya-codebase-security-scan` - Security scanning
...
```
freya-wrap-up --no-security    # Skip security scan
freya-wrap-up --no-docs        # Skip documentation update
freya-wrap-up --no-specs       # Skip specification update
freya-wrap-up --no-graph       # Skip dependency graph update
```

Multiple flags can be combined:

```
freya-wrap-up --no-security --no-specs "fix: typo in README"
```
...
You can then re-run `freya-wrap-up` after fixing the problem.

## When to Use

Use `freya-wrap-up` after:
