...
The read-only **check** counterpart of `freya-wrap-up` (which *does/syncs*).
...
| `status` (summary only) | Print the summary without rewriting `BACKLOG.md` — run `freya status` and omit `--write-backlog` |
...
code (`collect_status.py:366` opens it with mode `w`), so anything hand-authored at that
...
- `status` is read-only; use `freya-wrap-up` to actually sync/commit.
