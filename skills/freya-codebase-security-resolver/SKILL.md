---
name: freya-codebase-security-resolver
description: |
  ...
  implement → commit code → run freya-wrap-up
---
...
### `freya-codebase-security-resolver` (default)
...
2. Adding to spec (via freya-spec-manager):
...
After committing code changes, call `freya-wrap-up`. Wrap-up will:
...
### `freya-codebase-security-resolver list [--severity]`

Show findings without entering selection/planning.

**Severity filters:**
```
freya-codebase-security-resolver list              # All findings
freya-codebase-security-resolver list --critical   # Only critical
freya-codebase-security-resolver list --high       # Critical + high
freya-codebase-security-resolver list --medium     # Critical + high + medium
```
...
### `freya-codebase-security-resolver fix <ids...>`

Skip interactive selection, go directly to planning.

**Examples:**
```
freya-codebase-security-resolver fix SEC-001
freya-codebase-security-resolver fix SEC-001 SEC-003 SEC-007
```

**With severity shortcut:**
```
freya-codebase-security-resolver fix --critical   # Fix all critical
freya-codebase-security-resolver fix --high       # Fix all critical + high
```
...
9. Run `freya-wrap-up` for artifacts (Phase 12)

---

### `freya-codebase-security-resolver fix --dry-run`
...
To actually fix these, run:
  freya-codebase-security-resolver fix --critical
```
...
This skill uses freya-code-graph at multiple phases:
...
Commands used:
- `freya-code-graph dependents <file>` - Find files that import/use a file
- `freya-code-graph impact <file>` - Get blast radius for changes

### freya-spec-manager

This skill uses freya-spec-manager for metadata cleanup:
...
6. **Document intentional decisions** - If you choose not to fix a finding, document why in the spec using freya-spec-manager.
