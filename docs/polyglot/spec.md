...
| Clause | Status |
|---|---|
| §2.2 *"Default: graphify when installed and the project contains a language homegrown does not cover"* | **Superseded by CD-23, then refined by CD-26.** `auto` never *scores* — installing a binary on `PATH` must not change every blast radius on the machine. Since CD-26 it defers to a machine-level default the engineer answered once at install time, and falls back to the floor when there is none; the first build records that answer in the project's own committed settings so a clone and CI resolve the same backend |
