...
Every build/update that writes the graph runs one pruned tree walk, filtered by the build's own scope rule (`CodeGraph._should_exclude`, not `substrate.exclusions`) and by a two-tier extension model, and records the result at `graph["substrate"]["unmapped_source"]`. `graph.json` is gitignored (`git check-ignore -v` -> `.gitignore:18:**/.graph/`; `behavior.json` is deliberately not), so the block costs zero tracked diff on every machine. The walk lives in `_finalise` — the single funnel every backend passes through, and the only point after `update()`'s wholesale `graph['substrate']` rewrite at :2095-2099. Measured 0.0007s–0.0146s across four real repositories, against the 2.105s `_scan_files` already pays on the largest.
...
The key is **absent** when there is nothing to say. Two tiers draw that line: `SOURCE_EXTENSIONS` (closed-world, definite program source) is always reported; `SCRIPT_EXTENSIONS` (`.sh`, `.sql`, `.ps1`, …) only when its count exceeds `max(files_graphed, 2)`. Measured: silent on freya-devkit; `{".java": 12}` on the fixture; `{".mjs":3,".mts":2,".feature":1,".prisma":1}` on acme-site-testbed. (An earlier draft of this paragraph also cited `acme-lab`, which is an empty directory, and `acme-media`, whose silence turned out to be a defect rather than a filter working — see the review entry in `log.md`.) A closed world defaults to silence on the unknown, which is the correct default for a signal whose only value is being believed.
