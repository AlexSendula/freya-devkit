...
same way (`graph_ops.py:2534`). Every one of those fallbacks records `degraded_from` and its
...
relocatable with `FREYA_HOME` (`settings.py:59`, `settings.py:130`) — its own directory rather
...
Precedence is **project, then machine, then floor** (`settings.py:282`). An explicit name in a
...
states (`settings.py:296`, `settings.py:306`). The first `--build` or `--update` in a project
that has not decided copies the machine's answer into that project's own committed
`knowledge-base/settings.json` (`graph_ops.py:3093` → `settings.py:506`), validating it against
...
(`graph_ops.py:2985`), so it can only ever tell you about a tool you already own. Two other
...
auto-enables non-interactive mode whenever stdin is not a TTY (`graph_ops.py:3076`), which is
...
committed by default (`graph_ops.py:243`, ADR-017). The seeding is a seed and not a live link:
...
promising more than it does (`graph_ops.py:2961`).
...
(`skills/freya-behavior-runner/scripts/run_behaviors.py:316`). One honest gap remains and this
...
(`graph_ops.py:2910`) and that is the entire mechanism. A project that ignores the line keeps
...
- **Put the setting somewhere other than `knowledge-base/settings.json`.** Three candidates, each
  ...
  the cache's own `.gitignore` (`graph_ops.py:243`), so a setting recorded there works for
