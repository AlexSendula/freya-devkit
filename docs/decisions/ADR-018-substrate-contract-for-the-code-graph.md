...
(`substrate.py:572`) and imports no base class, because the second backend wraps a tool nobody
...
graph metadata as a degradation with the reason (`graph_ops.py:2992`). The check binds the call
itself, not just the attribute names — `BUILD_KWARGS` and `UPDATE_KWARGS` (`substrate.py:582`)
...
where it is visible (`substrate.py:741`). `unresolved:` is a prefix on the edge's *target*, not
a provenance value; `PROVENANCE` has exactly two members, `extracted` and `inferred`
(`substrate.py:57`), and the validator rejects a third (`substrate.py:724`). **Carry per-edge
...
no extensions at all (`substrate.py:646`), and `validate_graph` refuses a graph containing a
file outside the declared extensions (`substrate.py:788`). **Support incremental update, or
...
(`substrate.py:45`), `make_edge` raises on anything outside it (`substrate.py:132`), `Coverage`
raises on a declaration outside it (`substrate.py:401`), and `validate_graph` rejects an edge
...
Exclusions are a contract type, `substrate.Exclusions` (`substrate.py:456`), assembled by the
...
(`settings.py:72`). That correction, and the arguable-defaults problem behind it, are ADR-022.
...
(`project_shape.py:99`). Declaring coverage is what makes "no dependencies" and "this backend
...
backend receives (`graph_ops.py:437`).
