...
│ └── .graph/                                    IGNORED       │
│     ├── .gitignore            ← written by us   (contains *) │
│     ├── graph.json            ← code-graph      ignored      │
│     ├── classifications.json  ← code-graph      ignored      │
│     └── behavior.json         ← behavior-graph  ignored ⚠    │
└─────────────────────────────────────────────────────────────┘
```

`.graph/` ignores itself: `code-graph --build` writes a `.gitignore` containing `*` into it
(`skills/freya-code-graph/scripts/graph_ops.py:212`), so an adopting project never has to touch
its root `.gitignore`. The reasoning is that `graph.json` and `classifications.json` are a
**parse cache** — rebuildable from source in seconds, and large enough (124 KB on a ~230-file
app) that committing them would put an unreadable, conflict-prone diff in a large share of
commits.

> ⚠ **`behavior.json` is ignored by a rule written for its neighbours.** It is not a parse
> cache: it holds *observed* coverage fingerprints captured by running the test suite, so it
> cannot be regenerated by re-reading source — only by re-running the tests. At ~1.4 KB none of
> the churn reasoning applies to it either. Tracked in [backlog.md](backlog.md).
