...
### 7. `behavior.json` is git-ignored by a rule written for its neighbours

`code-graph --build` writes a `.gitignore` containing `*` into `knowledge-base/.graph/`
(`skills/freya-code-graph/scripts/graph_ops.py:212`), so everything in that directory is
ignored. That is right for `graph.json` and `classifications.json` — a parse cache, rebuildable
from source in seconds, and large enough (124 KB on a ~230-file project) that committing it
would put an unreadable, conflict-prone diff in a large share of commits.

`behavior.json` is a different kind of artifact and got swept up. It holds **observed** coverage
fingerprints captured by *running the test suite*, so it cannot be regenerated by re-reading
source — only by re-running the tests, which needs a working runner and a green suite. At ~1.4 KB
none of the churn or merge-conflict reasoning applies. A fresh clone therefore starts with no
observed coverage at all and silently degrades to `static`/`unknown` until someone runs the
suite, which is exactly the "coverage-unknown, never silent" failure mode the substrate work
(ADR-005) exists to prevent.

**Why it happened:** the ignore came out of dogfooding finding F8, which was framed as a bug
("the generated `.graph/` directory is not ignored") rather than as a per-file decision, so the
two artifact kinds were never separated.

**How to pick it up:** decide whether `behavior.json` moves out of `.graph/` (e.g. to
`knowledge-base/behavior.json`) or whether the written `.gitignore` becomes selective
