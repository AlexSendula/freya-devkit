...
written only when `--write-backlog` is passed (`collect_status.py:395`), and `wrap-up`
...
The write is a full overwrite — `open(path, "w")` at `collect_status.py:366` — with no read of
...
rendered census (`collect_status.py:366`). There is no merge, no backup, no
...
**Decision**: `render_backlog` reads `status["notes"]` (`collect_status.py:320`) and, when the
...
(`collect_status.py:322`–`:325`). A run that read every source emits no such block.
...
sample-capped for this reason (`UNRECOGNISED_SAMPLE`, `collect_status.py:166`): three hundred
...
| 2026-08-24 | Reversed the "notes do not reach the backlog" decision — `render_backlog` reads `status["notes"]` and renders them as a caveat block above the sections. Added BEH-160. Dropped the claim that the `:366` overwrite citation had drifted. | The rendering hole this spec called the weakest inference in it was closed on 2026-08-23: the `(value, note)` pair now reaches the committed artifact, which was the one place the ADR-005 caveat was being lost. The `:366` citation was re-measured and is correct. |
