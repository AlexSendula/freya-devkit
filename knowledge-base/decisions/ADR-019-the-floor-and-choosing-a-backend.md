...
same way (`graph_ops.py:2728`). Every one of those fallbacks records `degraded_from` and its
...
typed it wrong (`graph_ops.py:3404`). At machine scope, `--use auto` is not an answer — it
...
`knowledge-base/settings.json` (`graph_ops.py:3562` → `settings.py:968`), validating it against
...
(`graph_ops.py:3454`), so it can only ever tell you about a tool you already own. Two other
...
auto-enables non-interactive mode whenever stdin is not a TTY (`graph_ops.py:3545`), which is
...
promising more than it does (`graph_ops.py:3430`).
...
(`graph_ops.py:3379`) and that is the entire mechanism. A project that ignores the line keeps
...
- **Ask through the agent, using the deferred-prompt protocol.** This works headless, which the
  ...
  (`graph_ops.py:1901`) and, verified at `2762d54`, have no caller and no CLI flag anywhere in
...
- **Put the setting somewhere other than `knowledge-base/settings.json`.** Three candidates, each
  ...
  the cache's own `.gitignore` (`graph_ops.py:245`), so a setting recorded there works for
