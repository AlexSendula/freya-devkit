...
same way (`graph_ops.py:2753`). Every one of those fallbacks records `degraded_from` and its
...
(`graph_ops.py:3479`), so it can only ever tell you about a tool you already own. Two other
...
auto-enables non-interactive mode whenever stdin is not a TTY (`graph_ops.py:3570`), which is
...
promising more than it does (`graph_ops.py:3455`).
...
(`skills/freya-behavior-runner/scripts/run_behaviors.py:337`). One honest gap remains and this
...
(`graph_ops.py:3404`) and that is the entire mechanism. A project that ignores the line keeps
...
- **Ask through the agent, using the deferred-prompt protocol.** This works headless, which the
  ...
  (`graph_ops.py:1926`) and, verified at `2762d54`, have no caller and no CLI flag anywhere in
