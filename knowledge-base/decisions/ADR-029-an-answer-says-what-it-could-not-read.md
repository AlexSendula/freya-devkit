...
performed in `_finalise` (`skills/freya-code-graph/scripts/graph_ops.py:2572`)
...
build's *own* scope rule, `CodeGraph._should_exclude` (`graph_ops.py:1363`)
...
(`_announce_unmapped`, `graph_ops.py:3000`). The key is **absent** — not empty
...
(`graph_ops.py:1955`) globs by `FILE_PATTERNS`, so a file whose extension the
...
parses" (`graph_ops.py:2424-2396`). It had been applied to "the file you asked
...
not a TTY (`graph_ops.py:3290`), which is every agent-driven run and every
...
- **Wrap `--dependents` only, since nothing parses it.** Genuinely free: the
  ...
  (`graph_ops.py:3270`) and the dispatch that reads `args.dependents`
  (`graph_ops.py:3342`). It would have bought the payload signal on one of the
...
- **Bump `GRAPH_SCHEMA_VERSION` so the census's presence is implied, instead of
  ...
  (`graph_ops.py:2184-2168`), and that rebuild changes the graph the
