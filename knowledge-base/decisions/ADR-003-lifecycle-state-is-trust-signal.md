...
- **Dispatching on level/adapter before state.** A confirmed record may name a vitest locator for a test that does not exist yet. Pinned by `test_confirmed_with_unit_adapter_is_still_not_executed` (`skills/freya-behavior-runner/scripts/test_run_behaviors.py:327`) under the rule "state wins over level/adapter".
