...
that belongs to the contract. `run_build` and `run_update` (`graph_ops.py:2787`, `:2792`) pass
the result to one shared funnel, `_finalise` (`graph_ops.py:2623`), which derives the
`dependents` reverse index (`:2658`), refuses to overwrite a populated graph with an empty one
(`:2660`), validates what the backend emitted and records any errors in the artifact
(`:2666`–`:2683`), takes the census of in-scope files the backend cannot read (`:2691`), and
writes both `graph.json` and `graph.<backend>.json` (`:2715`, and ADR-028 for why there are two).
...
mishandled (`graph_ops.py:2626`). `project_dir` is a required backend attribute for this reason
...
leaves it alone (`graph_ops.py:2631`–`:2649`).
...
(`graph_ops.py:2733`), which lets the caller degrade to the floor and keeps the previous artifact
...
- **Put the shared half in `backends.py`, next to the registry.** Attractive on size alone:
  ...
  borrow its scope rule (`graph_ops.py:2876`).
...
- **`_refuse_to_erase` starts firing on legitimate builds.** It compares against the active
  artifact whoever wrote it (`graph_ops.py:2752`), so switching a repository from a polyglot
