...
that belongs to the contract. `run_build` and `run_update` (`graph_ops.py:2762`, `:2767`) pass
the result to one shared funnel, `_finalise` (`graph_ops.py:2598`), which derives the
`dependents` reverse index (`:2633`), refuses to overwrite a populated graph with an empty one
(`:2635`), validates what the backend emitted and records any errors in the artifact
(`:2641`–`:2658`), takes the census of in-scope files the backend cannot read (`:2666`), and
writes both `graph.json` and `graph.<backend>.json` (`:2690`, and ADR-028 for why there are two).
...
mishandled (`graph_ops.py:2601`). `project_dir` is a required backend attribute for this reason
...
leaves it alone (`graph_ops.py:2606`–`:2624`).
...
(`graph_ops.py:2708`), which lets the caller degrade to the floor and keeps the previous artifact
...
- **Put the shared half in `backends.py`, next to the registry.** Attractive on size alone:
  ...
  borrow its scope rule (`graph_ops.py:2851`).
...
- **`_refuse_to_erase` starts firing on legitimate builds.** It compares against the active
  artifact whoever wrote it (`graph_ops.py:2727`), so switching a repository from a polyglot
