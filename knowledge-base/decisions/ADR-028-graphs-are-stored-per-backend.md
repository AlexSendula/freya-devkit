...
(`skills/freya-code-graph/scripts/graph_ops.py:2516`, `:2529`;
...
(`graph_ops.py:2608`), so no backend can opt out and none can name the file differently.
...
`graph.*.json` beside it (`graph_ops.py:2463`, `:2474`), pinned by a test
...
they wrote earlier (`graph_ops.py:2087`–`:2189`; `backend_graphify.py:372`). So what this
...
`classifications.json` is deliberately spared by the same clear (`graph_ops.py:2470`) — it holds
...
anything else remains (`graph_ops.py:2474`–`:2484`). `behavior.json`, `classifications.json` and
