...
Append-only was chosen over the governance spec's own illustrated `status: active|superseded` mutable field. Flipping that field means rewriting a line, so the field and the append-only property are incompatible — and the property is worth more: an append-only log is tamper-evident, and its history is the record of how the team calibrated its own principles. The later-record form reproduces the spec's semantics exactly, so this is a faithful realization rather than a scope cut, guarded in every module by `test_append_is_append_only` plus a superseded-retirement test — `test_superseded_record_retires_the_pair` in `test_principles.py`, `test_superseded_retires_the_pair` in `test_contradictions.py` and `test_drift.py`, `test_superseded_drops_the_pair` in `test_resolution_log.py`.
